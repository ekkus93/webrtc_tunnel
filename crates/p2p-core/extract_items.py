...
A token is either an item name (fn/struct/enum/trait/type/const/static) or
`impl:Name` to target an `impl ... Name {` block. Captures preceding
attribute/doc lines, balances braces, removes the block from <src_rel>, and
appends it to <dst_rel>. Visibility/imports are fixed up afterward.
"""
import re
import sys
