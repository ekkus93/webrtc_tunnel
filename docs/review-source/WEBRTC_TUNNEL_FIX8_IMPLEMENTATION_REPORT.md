...
- **P0-001-D follow-up**: remove the retained `importIdentityPath` re-read branch; migrate path-based save tests to call `importIdentityFromPath()` (populates the draft) so `pathFileReplacementAfterValidationCannotChangeCommittedIdentity` holds.
