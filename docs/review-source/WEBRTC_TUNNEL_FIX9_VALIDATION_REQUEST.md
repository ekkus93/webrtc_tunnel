...
**Latest rerun requested after Android detekt cleanup:** current `master` at this commit.
...
The prior validation run `30344714809` failed in the Android job during `./gradlew --no-daemon check` because detekt reported FIX9 formatting/complexity findings. The follow-up commits fixed those findings without relaxing detekt configuration.
...
**Status:** rerun requested after Android detekt cleanup; not yet passed.
